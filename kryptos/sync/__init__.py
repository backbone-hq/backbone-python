...
class KryptosAuth(httpx.Auth):
    ...
    def auth_flow(self, request: httpx.Request) -> Generator[httpx.Request, httpx.Response, None]:
        ...
        if response.status_code == 401:
            request.headers["Authorization"] = f"Bearer {self.token}"
            self.kryptos.authenticate(self.permissions)
            yield request
...
class KryptosClient:
    ...
    def paginate(self, endpoint):
        response = self.client.get(endpoint, auth=self.authenticator)
        ...
        while result["next"]:
            response = self.client.get(endpoint, params=result["next"], auth=self.authenticator)
            response.raise_for_status()
            result = response.json()
...
class _TokenClient:
    ...
    def authenticate(self, permissions: List[Permission]) -> str:
        ...
        response = self.kryptos.client.post(
            token_endpoint,
            json={
    ...
    def derive(self) -> str:
        token_endpoint = self.kryptos.endpoint("token")
        response = self.kryptos.client.patch(token_endpoint, auth=self.kryptos.authenticator)
        response.raise_for_status()
        return self._parse(response)

    def revoke(self) -> None:
        token_endpoint = self.kryptos.endpoint("token")
        response = self.kryptos.client.delete(token_endpoint, auth=self.kryptos.authenticator)
        response.raise_for_status()
...
class _WorkspaceClient:
    ...
    def get(self) -> dict:
        ...
        response = self.kryptos.client.get(endpoint, auth=self.kryptos.authenticator)
        response.raise_for_status()
    ...
    def delete(self, safety_check=True) -> None:
        ...
        endpoint = self.kryptos.endpoint("workspace")
        response = self.kryptos.client.delete(endpoint, auth=self.kryptos.authenticator)
        response.raise_for_status()
...
class _EntryClient:
    ...
    def revoke(self, key: str, *users: PublicKey) -> None:
        endpoint = self.kryptos.endpoint("grant", "entry", key)
        response = self.kryptos.client.delete(
            endpoint,
