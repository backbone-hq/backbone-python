...
class TokenClient:
    ...
    async def get(self) -> models.Token:
    ...
    async def authenticate(self, permissions: Optional[List[models.Permission]] = None, duration: int = 86_400) -> str:
        challenge_response = await self.backbone.session.request(
            method="GET",
    ...
    async def derive(self, permissions: Optional[List[models.Permission]] = None, duration: int = 86_400) -> str:
        response = await self.backbone.session.request(
            method="POST",
