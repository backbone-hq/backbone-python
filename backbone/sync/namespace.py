...
class NamespaceClient:
    ...
    def get_child_namespaces(self, prefix: str) -> Iterable[dict]:
        for item in self.backbone.paginate(f"{self.child_namespace_endpoint}/{prefix}"):
            yield item

    def get_child_entries(self, prefix: str) -> Iterable[dict]:
        for item in self.backbone.paginate(f"{self.child_entry_endpoint}/{prefix}"):
            yield item
    ...
    def delete(self, key: str) -> None:
        ...
        chain = self.backbone.namespace.get_chain(f"{self.endpoint}/{key}")
